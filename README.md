...
Although I (Av Shrikumar) haven't implemented support for multiple threads, the process of generating explanations can be parallelized by sequence; it is advisable to split your input sequences file into multiple smaller files, running gkmexplain in parallel on each smaller file, and concatenate the results.
